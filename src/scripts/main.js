...
/**
 * Checks if an id is a fake id. This is the case when Teftish ignores you because of the `teftish_ignore` cookie.
...
export const attributes = function (detailed = false) {
	...
	const detailedData = {
		siteLanguage: (navigator.language || navigator.userLanguage).substr(
			0,
			2
		),
...
/**
 * Construct URL to the GraphQL endpoint of Teftish.
 * @param {String} server - URL of the Teftish server.
 * @returns {String} endpoint - URL to the GraphQL endpoint of the Teftish server.
 */
 ...
 * @param {String} url - URL to the GraphQL endpoint of the Teftish server.
...
/**
 * Looks for an element with Teftish attributes and executes Teftish with the given attributes.
 * Fails silently.
 */
export const detect = function () {
	const elem = document.querySelector("[data-teftish-domain-id]");

	if (elem == null) return;

	const server = elem.getAttribute("data-teftish-server") || "";
	const domainId = elem.getAttribute("data-teftish-domain-id");
	const options = elem.getAttribute("data-teftish-opts") || "{}";
...
export const create = function (server, options) {
	...
	// Fake instance when Teftish ignores you
	const fakeInstance = {
		record: () => ({ stop: noop }),
	...
	if (
		...
		console.warn("Teftish ignores you because you are on localhost");
		return fakeInstance;
	}

	if (isBot(navigator.userAgent) === true) {
		console.warn("Teftish ignores you because you are a bot");
		return fakeInstance;
	}
	...
	const _record = (domainId, attrs = attributes(options.detailed), next) => {
		...
		send(url, createRecordBody(domainId, attrs), options, (json) => {
			...
			if (isFakeId(recordId) === true) {
				return console.warn(
					"Teftish ignores you because this is your own site"
				);
			}
	...
	const _updateRecord = (recordId) => {
		...
		if (isFakeId(recordId) === true) {
			console.warn("Teftish ignores you because this is your own site");
			return { stop };
		}
...
// Only run Teftish automatically when executed in a browser environment
if (isBrowser === true) {
	detect();
}
